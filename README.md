...
- v0.1.2: `--append_mode` 参数更名为 `--gather_mode` 以增强语义，`--append_mode` 参数将作为 `--gather_mode` 的别称。
...
\*[5] 消息收集模式旨在于通过发送多条消息，实现多提示词拼接和图片收集，解决单条消息的局限性问题（例如图片和文本不能同时发送，或者消息平台只接受第一张图片等）。发送「开始」将使用收集的提示词和图片进行图片生成；发送「取消」可以取消操作。
